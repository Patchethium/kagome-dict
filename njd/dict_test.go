...
const (
	NJDDictEntrySize = 486757 + 302157
	testDictPath     = "./njd.dict"
)

func Test_DictShrink(t *testing.T) {
	d := DictShrink()
	if want, got := NJDDictEntrySize, len(d.Morphs); want != got {
...
func Test_New(t *testing.T) {
	...
	if want, got := NJDDictEntrySize, len(d.Contents); want != got {
		t.Errorf("want %d, got %d", want, got)
	}
}
